...
app.get("/handleFile", (req, res, err) => {
    if (req.session.user){
        ...
        res.render('filePage',{
            user: req.session.user[0].username,
            msg: 'The uploaded file doesn\'t have total = 100'
        });
      } else {
        // redirect ... back to login
        res.render('index',{
...
app.post("/checkUser", (req, res) => {
    ...
    db.query(sql2, (err, results) => {
        ...
        if (results.length === 0){
            ...
            console.log("USER RETRIEVED: ", results); // There has to one user with unique username and password in the database
...
app.post("/handleFile", (req, res, err) => {
    ...
    if (total_sum !== 100){
        console.log('total doesn\'t sum up to 100');
...
app.get("/createTable", (req, res) => {
    let sql =
      ...
      "(username VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, PRIMARY KEY(username));";
